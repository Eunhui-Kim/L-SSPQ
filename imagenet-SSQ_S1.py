...
import resnet_model3
from resnet_model3 import preact_group, resnet_backbone2, resnet_group, asl_group, resIBShift_group, QresIBShift_group
import numpy as np

class Model(ImageNetModel):
    def __init__(self, depth, mode='QresIBShift'):
        self.mode = mode
        basicblock = getattr(resnet_model3, mode + '_basicblock', None)
        bottleneck = getattr(resnet_model3, mode + '_bottleneck', None)
        self.group = getattr(resnet_model3, mode + '_group', None)
        ...
        if args.fake:
            ...
            logger.set_logger_dir(
                os.path.join('train_log',
                             'imagenet-{}-d{}-batch{}-ssplq-clr-0510-q48-TS1'.format(
