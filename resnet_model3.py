...
def beforeAP(name, l, block_func, features, count, stride):
    with tf.variable_scope(name):
        l = Conv2D('conv1x1a', l, features, kernel_size=1, strides=1)
        l = BatchNorm('stembn', l)
        l = activate(l)
    return l

def afterAP(name, l, block_func, features, count, stride):
    with tf.variable_scope(name):
        l = activate(l)
    return l
# -----------------Quntized ResIB-Shift resnet ----------------------
...
def resnet_backbone2(image, num_blocks, group_func, block_func):
    with argscope(Conv2D, use_bias=False,
                  kernel_initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out')):
        # Note that TF pads the image by [2, 3] instead of [3, 2].
        # Similar things happen in later stride=2 layers as well.
        ...
        l = group_func('group2', l, block_func, 256, num_blocks[2], 2)
        l = group_func('group3', l, block_func, 512, num_blocks[3], 2)
        l = beforeAP('bfAP', l, block_func, 512, num_blocks[3], 1)
        l = GlobalAvgPooling('gap', l)
        l = afterAP('afAP', l, block_func, 512, num_blocks[3], 1)
        logits = FullyConnected('linear', l, 1000,
                                kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    return logits
